	...
	public class Plane extends Mesh
	{
		...
		public function Plane(width:Number = 100, length:Number = 100, widthSegments:uint = 1, lengthSegments:uint = 1, twoSided:Boolean = true, reverse:Boolean = false, bottom:Material = null, top:Material = null)
		{
			if (widthSegments <= 0 || lengthSegments <= 0)
				return;
			var indices:Vector.<uint> = new Vector.<uint>(); //每个三角形的顶点的索引
			var x:int;
			var y:int;
			var wEdges:int = widthSegments + 1; //宽度边界数,为分段数+1
			var lEdges:int = lengthSegments + 1; //高度边界数,为分段数+1
			...
			for (x = 0; x < wEdges; x++)
			{
				for (y = 0; y < lEdges; y++)
				{
					vertices.writeFloat(x * segmentWidth - halfWidth); //x坐标
					vertices.writeFloat(y * segmentLength - halfLength); //y坐标
					vertices.writeFloat(0); //z坐标
			...
			for (x = 0; x < wEdges; x++)
			{
				for (y = 0; y < lEdges; y++)
				{
					if (x < widthSegments && y < lengthSegments)
					{
						//这里创建了两个三角形,及创建了一个矩形
						//x,y实际代表矩形在网格中的坐标
						createFace(indices, vertices, x * lEdges + y, //a,左下点index
							(x + 1) * lEdges + y, //b,
							(x + 1) * lEdges + y + 1, //c
							x * lEdges + y + 1, //d
							0, //nx
